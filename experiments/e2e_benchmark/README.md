...
- **네트워크가 되는 환경에서 실행해야 한다.** 이 코드를 만든 sandbox 환경은 `api.groq.com`이
  네트워크 허용 목록에 없어 자체적으로 실행이 불가능했다 — 로컬 머신(Terminal.app)에서 실행하세요.
- `export GROQ_API_KEY=...` (Groq 콘솔에서 발급). **이 키를 코드나 커밋에 절대 넣지 마세요.**
  환경변수로만 전달합니다.
...
- `estimated_cost_usd_approx`는 `harness.py`에 하드코딩된 근사 단가다. 실제 청구액이 아니다
  (`GROQ_PRICE_INPUT_PER_1M`, `GROQ_PRICE_OUTPUT_PER_1M` 환경변수로 조정 가능).
